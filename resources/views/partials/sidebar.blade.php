...
<nav class="glass glass--rail p-2.5 max-sm:p-1.5" x-sheen
     x-data="{ compact: JSON.parse(localStorage.getItem('rail-compact') ?? 'false') }"
     :class="compact && 'w-auto'">
        ...
        @foreach ($items as $item)
            ...
            <li class="max-sm:flex-1">
                <a href="{{ route($item['route']) }}"
                   class="nav-item max-sm:justify-center max-sm:px-2 {{ $active ? 'nav-item--active' : '' }}"
                   title="{{ $item['label'] }}">
    ...
    <button type="button"
            ...
            @click="compact = !compact; localStorage.setItem('rail-compact', compact); $nextTick(() => window.dispatchEvent(new Event('resize')))"
